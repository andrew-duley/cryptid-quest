// Script to optimize game element images
...
// These are the widths we'll generate for each game element image
const TARGET_WIDTHS = [128, 192, 256, 384, 512];
...
async function processAssetDir(assetPath) {
  ...
  if (!fs.existsSync(assetDir)) {
    console.warn(`⚠️  Game image folder not found: ${assetDir}`);
    return;
  }
...
(async () => {
  ...
  if (!assetPath) {
    console.error("Usage: npm run optimize:game-element-image -- <assetPath>");
    process.exit(1);
  }
  ...
  try {
    ...
    console.error('❌ Error optimizing game element image:', error);
    process.exit(1);
  }
})();